...
#### Швидкий старт (стандартна конфігурація)
...
#### Запуск з шардуванням (Lab 6)

Для запуску з шардуванням використовуйте окремий docker-compose файл:

1. **Запустіть контейнери з шардуванням**:
   ```bash
...
3. **Архітектура шардування**:
   - 4 вузли PostgreSQL (postgres_00..postgres_03)
   - 16 баз даних (db_0..db_f)
   - Автоматичне визначення шарду на основі UUID

4. **Примітка**: При шардуванні запити до списків повертають дані тільки з першого шарду.
...
#### Сервіси (стандартна конфігурація)
...
#### Сервіси (з шардуванням)

- **postgres_00..postgres_03**: 4 вузли PostgreSQL
  - Порти: 5400, 5401, 5402, 5403 (localhost)
  - Кожен вузол містить 4 бази даних
  - Дані зберігаються в окремих volumes

- **app**: Travel Plans API додаток з підтримкою шардування
  - Автоматично визначає шард на основі UUID
  - Використовує mapping.json для конфігурації підключень
...
- ✅ Шардування (Lab 6): підтримка 16 баз даних з автоматичним визначенням шарду

## Шардування (Lab 6)

Проект підтримує шардування баз даних для масштабування. Детальна інформація в `lab6-report.txt`.

### Основні компоненти

- **docker-compose-sharding.yml**: Конфігурація для запуску з шардуванням
- **mapping.json**: Конфігурація підключень до 16 баз даних
- **app/sharding.py**: Модуль для управління шардуванням
- **db/sharding/init-databases.sh**: Скрипт ініціалізації баз даних

### Алгоритм шардування

Визначення шарду відбувається на основі останнього символу UUID:
- UUID закінчується на '0' → db_0 (postgres_00)
- UUID закінчується на '1' → db_1 (postgres_00)
- ...
- UUID закінчується на 'f' → db_f (postgres_03)
...
## Оптимізація зберігання (Lab 8)

В рамках лабораторної роботи №8 виконано міграцію на використання типу даних **JSONB** для зберігання локацій.

- Таблиця `locations` видалена.
- Локації зберігаються як JSON-масив у колонці `travel_plans.locations`.
- Це забезпечує кращу продуктивність читання та спрощує роботу з даними в шардованому середовищі.
